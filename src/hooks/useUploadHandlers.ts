...
export const useUploadHandlers = (state: ReturnType<typeof useUploadState>) => {
  ...
  const markStepCompleted = useCallback(
    (step: number) => {
      state.setCompletedSteps((prev: Set<number>) => new Set(prev).add(step));
    },
    [state],
  );
  ...
  const validateFile = useCallback((f: File): boolean => {
    ...
    if (!validFileNamePattern.test(fileNameWithoutExt)) {
      alert(
        "파일명에는 한글, 영문, 숫자, 공백, 그리고 . - _ ( ) [ ] % 기호만 사용할 수 있습니다.",
      );
      return false;
    }
  ...
  // JSON 파싱
  const parseQuestionJson = useCallback(
  ...
  const parseErrorResponse = (
    ...
    type: "short_text" | "invalid_file" | "generation_failed" | "unknown";
    message: string;
  } => {
    const errorMessage =
    ...
    if (
      errorMessage.includes("텍스트가 너무 짧습니다") ||
      errorMessage.includes("최소 200자")
    ) {
      return { type: "short_text", message: errorMessage };
    }
    if (
      errorMessage.includes("텍스트를 추출할 수 없습니다") ||
      errorMessage.includes("파일에서")
    ) {
  ...
  const handleGenerateSummary = useCallback(async () => {
    ...
    if (state.sumTab === 2)
      formData.append("topic_count", String(state.sumTopicCount));
    if (state.sumTab === 4) {
      formData.append("keyword_count", String(state.sumKeywordCount));
      if (state.sumKeywordCount > 0) {
        const validKeywords = state.keywords.filter(
