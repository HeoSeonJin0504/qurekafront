...
export default function QuestionSolvePage() {
  ...
  const handleCreateFolder = async () => {
    ...
    try {
      ...
      setSnackbar({
        open: true,
        message:
          e.response?.data?.message || "폴더 생성 중 오류가 발생했습니다.",
        severity: "error",
      });
    }
  };
  ...
  const handleDeleteFolder = async () => {
    ...
    try {
      ...
      setSnackbar({
        open: true,
        message:
          e.response?.data?.message || "폴더 삭제 중 오류가 발생했습니다.",
        severity: "error",
      });
    } finally {
  ...
  const handleMoveQuestion = async () => {
    ...
    try {
      ...
      setSnackbar({
        open: true,
        message: e.response?.data?.message || "이동 중 오류가 발생했습니다.",
        severity: "error",
      });
    }
  };
  ...
  const handleConfirmDeleteFavorite = async () => {
    ...
    try {
      ...
      setSnackbar({
        open: true,
        message:
          e.response?.data?.message || "즐겨찾기 삭제 중 오류가 발생했습니다.",
        severity: "error",
      });
    } finally {
  ...
  return (
    <PageWrapper>
      ...
      <Inner>
        {!solveMode ? (
          <>
            ...
            <Paper
              elevation={2}
              sx={{ mb: 4, p: { xs: 2, sm: 3 }, borderRadius: 2 }}
            >
              <Typography
                variant="h6"
                gutterBottom
            ...
            <Box mb={5}>
              ...
              <CardList>
                {filteredFavorites
                  ...
                  .map((item, index) => (
                    <QuestionCard
                      key={`fav-${item.id}-${item.questionIndex ?? index}-${index}`}
                    ...
                    >
                      ...
                      >
                        <StarIcon
                          sx={{ fontSize: 18, color: "#FFD700", flexShrink: 0 }}
                        />
                        <Typography
                          fontWeight={600}
                          sx={{
      ...
      >
        ...
        >
          {snackbar.severity === "success" && (
            <CheckCircleOutline sx={{ fontSize: 22, color: "#1a5d3a" }} />
          )}
          <Typography
            sx={{ fontSize: "0.95rem", fontWeight: 500, flexGrow: 1 }}
          >
            {snackbar.message}
          </Typography>
          <IconButton
          ...
          {folders.map((folder) => (
            ...
            >
              <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
                <FolderIcon
                  color={
                    targetFolderId === folder.folder_id ? "primary" : "action"
                  }
                />
      ...
      >
        ...
        <DialogContent>
          <Typography>
            "{folderToDelete?.folder_name}" 폴더를 삭제하시겠습니까?
          </Typography>
        </DialogContent>
        <DialogActions sx={{ px: 3, pb: 2 }}>
          <Button onClick={() => setDeleteFolderConfirmOpen(false)}>
            취소
          </Button>
          <Button
            variant="contained"
      ...
      >
        <MenuItem
          onClick={() => {
            setFolderToDelete(selectedFolderForMenu);
